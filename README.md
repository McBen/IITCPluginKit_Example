# IITCPluginKit_Example
This is a tutorial project for https://github.com/McBen/IITCPluginKit

## Tutorial: 
https://McBen.github.io/IITCPluginKit_Example

## Example Plugin 
### Count Portals on Path:
![example](https://mcben.github.io/IITCPluginKit_Example/images/final.png)  
Download: https://github.com/McBen/IITCPluginKit_Example/raw/master/dist/iitc_plugin_CountPortals.user.js

#### Other code examples:  
- https://github.com/McBen/IITCPluginKit_Imageoverlay
- https://github.com/McBen/IITCPluginKit_CopyGPS

### Documentation

The documentation for this tutorial lives in the `docs` folder. It is written in markdown and is processed with [Vitepress](https://vitepress.dev).

To render it locally run:

```shell
yarn run docs:dev
```
